...
export const findNearestHelper = (serviceType, userLat, userLng) => {
    return new Promise((resolve) => {
        setTimeout(() => {
            ...
            // FALLBACK: If no specific helper found, generate a generic one to ensure "Everyone Available"
            if (!nearest) {
                const randomDist = (0.5 + Math.random() * 1.5).toFixed(2);
                nearest = {
                    id: `gen_${Math.random()}`,
                    name: `${serviceType} Expert`,
                    service: serviceType,
